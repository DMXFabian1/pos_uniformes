...
def build_inventory_tab(window: "MainWindow") -> QWidget:
    ...
    new_category_action.setToolTip("Crear una nueva categoria de productos (ej. Uniformes, Accesorios)")
    new_category_action.triggered.connect(window._handle_create_category)
    new_brand_action = new_menu.addAction("Marca")
    new_brand_action.setToolTip("Crear una nueva marca (ej. Fertoni, Ykan)")
    new_brand_action.triggered.connect(window._handle_create_brand)
    new_menu.addSeparator()
    new_product_action = new_menu.addAction("Producto")
    new_product_action.setToolTip("Crear un producto base con nombre, escuela, tipo de prenda y pieza")
    new_product_action.triggered.connect(window._handle_create_product)
    new_variant_action = new_menu.addAction("Presentacion")
    new_variant_action.setToolTip("Crear una presentacion nueva (talla, color, precio) para un producto existente")
    ...
    duplicate_variant_action.setToolTip("Copiar la presentacion seleccionada como base para crear una nueva")
    ...
    edit_product_action.setToolTip("Modificar nombre, escuela, marca o tipo de prenda del producto seleccionado")
    ...
    edit_variant_action.setToolTip("Modificar SKU, talla, color o precio de la presentacion seleccionada")
    ...
    bulk_price_action.setToolTip("Cambiar el precio a multiples presentaciones de una vez con una tabla editable")
    ...
    stock_menu.setToolTipsVisible(True)
    register_entry_action = stock_menu.addAction("Registrar entrada")
    register_entry_action.setToolTip("Registrar mercancia recibida de proveedor sumando unidades al stock")
    ...
    physical_count_action.setToolTip("Contar piezas fisicas y ajustar diferencias contra el stock del sistema")
    ...
    correct_stock_action.setToolTip("Ajustar manualmente el stock de una presentacion con referencia y motivo")
    ...
    bulk_adjust_action.setToolTip("Ajustar stock de multiples presentaciones de una vez con una tabla editable")
    ...
    labels_menu.setToolTipsVisible(True)
    print_selected_action = labels_menu.addAction("Imprimir seleccion")
    print_selected_action.setToolTip("Generar e imprimir etiquetas para las filas seleccionadas en la tabla")
    print_selected_action.triggered.connect(window._handle_inventory_print_label)
    print_visible_action = labels_menu.addAction("Imprimir resultados visibles")
    print_visible_action.setToolTip("Generar etiquetas para todas las presentaciones que coincidan con los filtros activos")
    ...
    delete_product_action.setToolTip("Eliminar permanentemente el producto y todas sus presentaciones")
    ...
    delete_variant_action.setToolTip("Eliminar permanentemente la presentacion seleccionada (SKU, talla, color)")
    delete_variant_action.triggered.connect(window._handle_delete_variant)
    more_menu.addSeparator()
    qr_lote_action = more_menu.addAction("Generar QR de seleccion")
    qr_lote_action.setToolTip("Generar codigos QR para las presentaciones seleccionadas en la tabla")
    qr_lote_action.triggered.connect(window._handle_generate_all_qr)
    qr_missing_action = more_menu.addAction("Generar QR faltantes")
    qr_missing_action.setToolTip("Generar QR solo para presentaciones activas que aun no tienen archivo QR")
    qr_missing_action.triggered.connect(window._handle_generate_missing_qr)
    qr_regen_action = more_menu.addAction("Regenerar todos los QR")
    qr_regen_action.setToolTip("Recrear todos los QR de presentaciones activas (sobreescribe los existentes)")
    qr_regen_action.triggered.connect(window._handle_regenerate_all_qr)
    window.inventory_more_button.setMenu(more_menu)

    window.inventory_bulk_adjust_button.setVisible(False)
    window.inventory_bulk_price_button.setVisible(False)
    toolbar = QHBoxLayout()
    ...
    window.inventory_precio_text_input.setPlaceholderText("$ exacto")
    ...
    window.inventory_precio_filter_combo.clear()
    window.inventory_precio_filter_combo.addItem("Precio: todos", "")
    window.inventory_precio_filter_combo.addItem("Hasta $99", "under_100")
    window.inventory_precio_filter_combo.addItem("$100 – $199", "100_199")
    window.inventory_precio_filter_combo.addItem("$200 – $499", "200_499")
    window.inventory_precio_filter_combo.addItem("$500+", "500_plus")
    ...
    filters_row.addWidget(window.inventory_precio_text_input, 0, 6)
