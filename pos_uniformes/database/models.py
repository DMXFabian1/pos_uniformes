...
class LibretaVenta(Base):
    ...
    # Privado del dueño: el encargado no ve el DINERO de este cobro en
    # ninguna parte (ticket, pantalla, celular). Las comisiones y piezas sí
    # cuentan: la empleada cobra lo que trabajó. Solo cobros con TARJETA:
    # ocultar efectivo descuadraría el corte del cajón.
...
class AfluenciaHora(Base):
    """Personas que entran (y pasan por fuera) por cámara y hora.

    La escribe `afluencia/contador_afluencia.py` desde las cámaras del DVR
    (upsert aditivo cada minuto). El kiosko la cruza con `LibretaVenta` para
    mostrar entradas vs ventas por hora.
    """

    __tablename__ = "afluencia_hora"
    __table_args__ = (UniqueConstraint("camara", "hora", name="uq_afluencia_hora_camara_hora"),)

    id: Mapped[int] = mapped_column(primary_key=True)
    camara: Mapped[str] = mapped_column(String(60), nullable=False)
    # Inicio de la hora (timestamptz).
    hora: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, index=True)
    entradas: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0")
    salidas: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0")
    pasan: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0")
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
