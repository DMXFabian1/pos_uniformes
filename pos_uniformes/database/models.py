...
class Variante(Base):
    ...
    stock_bodega: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    stock_piso: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    ...
    def stock_tienda(self) -> int:
        """Stock disponible en estantes de tienda (total - bodega - piso)."""
        return self.stock_actual - self.stock_bodega - self.stock_piso
    activo: Mapped[bool] = mapped_column(default=True, nullable=False)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        server_default=func.now(),
