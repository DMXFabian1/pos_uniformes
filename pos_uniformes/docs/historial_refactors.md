...
- `2026-05-14`: feat — ranking de empleadas. Nuevo `employee_ranking_service` que agrega ventas `EMPLOYEE` por código en un rango de fechas y ordena por monto. Dialog "Ranking" en Empleadas muestra tabla `# / Empleada / Tickets / Piezas / Monto / Última venta` con periodos Hoy / 7 días / Este mes y toggle de montos. 10 tests. Commit `7be0720`. Checkpoint `validated-tests`.
...
- `2026-04-23`: cliente en presupuesto solo por QR — `quote_client_combo` deshabilitado (display only), etiqueta "Cliente (QR)", señal de selección manual eliminada. Botón "Nuevo cliente" conservado. Aplica en `quotes_view.py` y `quote_satellite_window.py`. Checkpoint `validated-tests`.
- `2026-04-22`: impresion de ticket de presupuesto en satelite queda funcional en impresora termica EC Line EC-PM-80320 (80mm). El problema raiz era que `QTextDocument.setTextWidth` usa coordenadas de pantalla y al pasar a la QPrinter (203 DPI) el ancho se interpretaba mal, dejando el texto en una franja angosta. La solucion final usa `QPainter.drawText` con `painter.viewport()` directamente, que opera en el sistema de coordenadas nativo del driver. Se quito `QPrintDialog` para imprimir directo a `impresora_preferida` sin que el dialogo sobrescriba la configuracion de pagina. `TICKET_PAPER_WIDTH_MM` pasa de 58 a 80mm. Separadores del presupuesto ajustados a 42 chars y `QUOTE_TERMS_WRAP_WIDTH` a 42. Archivos: `ui/dialogs/printable_text_dialog.py`, `ui/helpers/ticket_print_layout_helper.py`, `services/quote_text_service.py`. Checkpoint `validated-manual` en Windows con EC-PM-80320.
- `2026-04-22`: corregido error `DuplicateTable` en migracion `a3c7e9f1b204` al aplicar `upgrade heads` en Windows. `op.add_column` con `index=True` ya crea el indice; el `op.create_index` explícito posterior fallaba si el indice existia. Se agrego `if_not_exists=True` a los tres `create_index` de esa migracion. Nota operativa: con dos migraciones partiendo del mismo padre, el comando correcto es `alembic upgrade heads` (plural). Checkpoint `validated-manual` en Windows.
- `2026-04-18`: `Corte de caja` ahora desglosa descuentos y ajuste por redondeo: `ResumenCaja` suma `total_descuentos` (suma de `venta.descuento_monto`) y `total_ajuste_redondeo` (extraido de la observacion de cada venta). El detalle del corte en `Configuracion` muestra ambas lineas. `settings_cash_history_detail_helper.py` recibe los nuevos parametros; `main_window.py` los pasa desde `resumen`. Checkpoint `validated-tests`.
- `2026-04-18`: verificado que cliente en Caja ya es solo por escaneo de QR: `sale_client_combo` es siempre invisible y deshabilitado; el cliente solo se enlaza via `_apply_scanned_client_to_sale`; sin escaneo la venta queda en Mostrador; `confirm_replace` pide confirmacion al cambiar cliente con carrito activo. Item cerrado en backlog.
- `2026-04-18`: verificado que la calculadora de cobro ya responde al teclado fisico desde `fc5f0bb` (2026-03-12): digitos 0-9 escriben monto, `Backspace` corrige, `Enter` confirma y `Esc` cancela. No se requieren cambios adicionales. Item cerrado en backlog.
- `2026-04-10`: `Conteo fisico` hace mas explicito el modo `uno a uno`: cuando arranca sin lote previo, el campo de SKU y el boton se comportan como captura pieza por pieza (`Sumar 1`), el resumen visible muestra piezas contadas vs sistema base y aparece `Restar 1` como correccion rapida sobre la fila seleccionada. La base sigue siendo el mismo flujo de escaneo acumulado; solo se vuelve mas claro y usable para conteos de piso con pistola lectora. Checkpoint `validated-tests`; validacion manual en Windows queda `pending-manual`.
- `2026-04-10`: `Caja` abre una `V1` de `Agregar sin codigo` orientada a cobro rapido: el boton del header ya no compite con el escaneo y abre un popup minimo con `Precio` obligatorio y `Nombre rapido` opcional. La linea entra al carrito y al ticket como venta manual (`SIN-CODIGO` / `Venta manual`) sin afectar inventario, y mantiene ajuste de cantidad desde el carrito. Checkpoint `validated-tests`; validacion manual en Windows queda `pending-manual`.
