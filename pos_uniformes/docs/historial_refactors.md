...
- `2026-04-08`: `Conteo fisico` abre su primer corte de `V2` con checkpoint `validated-tests`: si `Inventario` tiene filas seleccionadas, el dialogo arranca con ese lote base ya sembrado y permite ajustar el `Contado` por fila dentro de la misma tabla, manteniendo intacto el flujo rapido por `SKU`. La extension a `todo lo filtrado` queda separada como siguiente paso para no precargar lotes accidentales demasiado grandes. Validacion manual en Windows queda `pending-manual`.
